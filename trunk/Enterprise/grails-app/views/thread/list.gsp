...
<table border="1" width="100%">
  ...
  <tr>
    <td style="vertical-align: top"><a href="/Enterprise/course/show?id=${thread.id}">${thread.name}</a>
      <br><ul>
        <g:each var="argument" in="${thread.arguments}">
          <li><g:link action="show" id="${post.id}">${post.name}</g:link>      </li>
        </g:each>
      </ul>
    </td>
    <td>
<!--  <g:message code="forumgrails.arguments"/>: ${course.arguments.size()}<br>
  ...
  <g:if test="${session.user}">
    <td>
    <g:link action="remove" id="${thread.id}"><g:message code="forumgrails.remove" args="${[thread.name]}"></g:message></g:link>
    </td>
  </g:if>
  </tr>
</g:each>
